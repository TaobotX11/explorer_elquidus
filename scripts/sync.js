...
function get_market_price(market_array) {
  ...
  } else if (settings.coin.symbol == 'NUX' && settings.markets_page.market_price == 'AVERAGE') {
    const coingecko = require('../lib/apis/coingecko');
    const currency = lib.get_market_currency_code();

    coingecko.get_market_prices_qutrade(settings.coin.symbol, function (err, last_price, last_usd_price) {
      // check for errors
      if (err == null) {
        Stats.updateOne({ coin: settings.coin.name }, {
          last_price: last_price,
          last_usd_price: last_usd_price
        }).then(() => {
