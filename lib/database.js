...
///[a-z\D]{5,}#\d{4}$/ms regex
function find_claim_name_media(hash, cb) {
  ClaimAddress.findOne({ a_id: hash, claim_name: { $regex: /[a-z\D]{5,}#\d/ } }).then((claim_address) => {
    if (claim_address)
      if (claim_address.a_id == hash)
        return cb(claim_address.a_id);
      else
        return cb('');
    else
      return cb();
...
module.exports = {
  ...
  get_claim_name_media: function (hash, cb) {
    find_claim_name_media(hash, function (claim_name) {
      return cb(claim_name);
    });
  },
  ...
  get_media: function (start, length, cb) {
    // get the count of orphaned blocks
    ClaimAddress.find({}).countDocuments().then((count) => {
      // get the actual orphaned block data
      ClaimAddress.find({}).sort({ a_id: -1 }).skip(Number(start)).limit(Number(length)).exec().then((media) => {
        return cb(media, count);
      }).catch((err) => {
        console.log(err);
        return cb([], count);
      });
