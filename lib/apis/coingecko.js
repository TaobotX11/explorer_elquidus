...
const base_url_qutrade = 'https://qutrade.io/api/v1';
...
function get_simple_price_qutrade(coin, cb) {
  request({ uri: base_url_qutrade + '/market_data/?pair=' + coin.toLowerCase() + '_usdt', json: true }, function (err, resp, body) {
    if (err)
      cb(err, 0, 0);
    else if (body == null || body == '' || typeof body !== 'object')
      ...
      try {
        ...
        last_price = parseFloat(body.list.nux_usdt.price);
        last_usd_price = parseFloat(body.list.nux_usdt.price)
        return cb(null, last_price, last_usd_price);
      } catch (err) {
        return cb('Received unexpected API qutrade data response', 0, 0);
      }
    }
  });
}
...
module.exports = {
  ...
  get_market_prices_qutrade: function (currency, cb) {
    get_simple_price_qutrade(currency, function (err, last_price, last_usd) {
      if (last_price == null)
        console.log(`Error: "${currency}" is not a valid qutrade api currency`);
